...
def animate_frame(x_start, x_end, y, frame, duration=0.1, main_window=None):
    steps = 15  # Stała liczba kroków
    ...
    if direction == 'next' and (next_page * items_per_page < total_items):
        for i in range(items_per_page):
            if current_page * items_per_page + i < total_items:
                ...
                threading.Thread(target=animate_frame, args=(45, -580, ((i % items_per_page) * height) + 10, frame, 0.15, main_window)).start()
        if (next_page + 1) * items_per_page < total_items:
            for i in range(items_per_page):
                if (next_page + 1) * items_per_page + i < total_items:
                    ...
                    threading.Thread(target=animate_frame, args=(1250, 670, ((i % items_per_page) * height) + 10, frame, 0.15, main_window)).start()
        ...
        for i in range(items_per_page):
            if current_page * items_per_page + i < total_items:
                ...
                threading.Thread(target=animate_frame, args=(1250, 45, ((i % items_per_page) * height) + 10, frame, 0.15, main_window)).start()
    elif direction == 'prev' and (current_page > 0):
        for i in range(items_per_page):
            if current_page * items_per_page + i < total_items:
                frame = item_frames[current_page * items_per_page + i]
                threading.Thread(target=animate_frame, args=(45, 670, ((i % items_per_page) * height) + 10, frame, 0.15, main_window)).start()
        current_page = next_page
        show_page(current_page)
        for i in range(items_per_page):
            if current_page * items_per_page + i < total_items:
                ...
                threading.Thread(target=animate_frame, args=(-590, 45, ((i % items_per_page) * height) + 10, frame, 0.15, main_window)).start()
        if current_page > 0:
            for i in range(items_per_page):
                if (current_page - 1) * items_per_page + i < total_items:
                    frame = item_frames[(current_page - 1) * items_per_page + i]
                    threading.Thread(target=animate_frame, args=(-1110, -580, ((i % items_per_page) * height) + 10, frame, 0.15, main_window)).start()
